/* Appwerk — high-fidelity draft app screens (v2, dribbble-grade).
   Rendered inside phone frames on index + detail pages. Each app carries its
   own product branding via CSS vars (--a/--d/--al) on .ph; German UI copy —
   these are product drafts for the DACH market, marked as changeable.
   window.SCREENS[appId] = [screenHTML ×3] */

(function () {
  const STATUS = `<div class="ph-status"><span>9:41</span><span class="ph-sig"><i></i><i></i><i style="width:13px"></i></span></div>`;
  const nav = (icons, on) => `<div class="ph-nav">${icons.map((ic, i) => `<span class="${i === on ? "on" : ""}">${ic}</span>`).join("")}</div>`;
  const appbar = (initials, greet, name) => `
    <div class="ph-appbar">
