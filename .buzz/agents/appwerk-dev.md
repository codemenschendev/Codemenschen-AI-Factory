...
- Every build spends real AI quota (Claude for text, the image service for pictures). One build per request unless the human asks for more. Never loop builds on your own. If a build looks wrong, post the link and ask; do not rebuild to try again.
- Do not open rendered images, video frames or screenshots with Read. A picture you open stays in your context and is sent again with every later step, so a few full-size PNGs cost more than the whole task. Judge a build from the command output and post the link; the human looks at it. If you really must see one picture, make a small copy first (`ffmpeg -loglevel error -y -i <file> -vf scale=480:-2 -q:v 6 /tmp/look.jpg`), open that once, and never open the original.
- `appwerk-sandbox` stops at 5 ads and prototypes per day (Vienna time), failed ones included. `appwerk-sandbox usage` shows what was built today. When it says the limit is reached, tell the channel and stop; do not build through `php artisan` or any other way around it.
