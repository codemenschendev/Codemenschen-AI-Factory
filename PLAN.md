...
- **Custom ideas:** `POST /api/quotes` runs a two-layer calculator: (1) Claude structures the idea into features/integrations/complexity (structured output, stored in `quotes.structured_spec`); (2) the deterministic estimator (server-side port of `create.html`'s `estimate()`, price sheet lowered 2026-08-27: base €200 web / €300 mobile / €450 both + feature add-ons €5–20, B2B multiplier, ×1.2, clamp €149–1,500; the ad-budget suggestion is informational and not part of the price) prices it. Scope summary is presented before checkout. No build before payment.
- **Appwerk Care** (2026-08-27): €9/month per app as a Stripe subscription (`care:<project>` checkout, `customer.subscription.*` webhooks mirrored to `projects.care_status`) — unlimited change rounds (`change_request_mode = care`, `change_requests.covered_by = care`, free rounds untouched), cancel any time, ends with the billing month. The €39 single round stays for customers without Care.
