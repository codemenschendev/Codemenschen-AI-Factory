...
- **Custom ideas:** `POST /api/quotes` runs a two-layer calculator: (1) Claude structures the idea into features/integrations/complexity (structured output, stored in `quotes.structured_spec`); (2) the deterministic estimator (server-side port of `create.html`'s `estimate()`, price sheet lowered 2026-08-27: base €400 web / €600 mobile / €900 both + feature add-ons €40–150, B2B multiplier, ×1.2, clamp €300–3,000; the ad-budget suggestion is informational and not part of the price) prices it. Scope summary is presented before checkout. No build before payment.
